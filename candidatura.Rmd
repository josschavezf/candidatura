...
- Apertura de la región -10 para formar la burbuja de transcripción.
...
```
](https://www.semanticscholar.org/paper/Functional-complexity-and-regulation-through-RNA-Dethoff-Chugh/13fc1bad70f3448b8617d83347da1e783d5b9ad6)
Dethoff, *et al*, 2012

[
```{r,echo=FALSE,out.width='90%'}
library(knitr)
include_graphics("img/ribotpp.png")
```
](https://link.springer.com/chapter/10.1007/978-981-10-8372-3_3)
Yamauchi & Sugimoto, 2018
]
...
Los organismos patógenos y parásitos estrictos mostraron un bajo número de TFs.
...
```{r,echo=FALSE}
x <- read.table("data/table_cog.txt", header = TRUE)
kable(x[c(1,2,5),c(1,5:8)], row.names = FALSE,align = 'c') %>% 
...
kable(x[c(1,2,5),-6], row.names = FALSE, align ='c') %>%
    kable_styling(position = "center")%>% 
...
```{r,echo=FALSE}
x <- read.table("data/table_cog.txt", header = TRUE)
kable(x[,c(1,2,5:8)], row.names = FALSE,align = c('c')) %>% 
...
```{r,echo=FALSE}
x <- tibble(x = c("Número de reguladores COG0568 en Proteobacterias", "Número de reguladores COG0568 en otros phyla"), y = c("Número de reguladores excepto COG0568 en Protebacterias", "Número de reguladores excepto COG0568 en otros phyla") )
...
<br><br>
.center[**Factores transcripcionales**]

```{r,echo=FALSE,out.width='100%',fig.align='center'}
include_graphics("img/F6_1.png")
```
...
- CodY (COG4465)
- ComK (COG4903)
- TFs predichos con dominios CBS (COG4109)
...
- MarR con dominio helix-turn-helix 
- MerR 
...
```
.pull-left[
- Familia AcrR en Actinobacterias
- Familia LysR en Proteobacteras
- Familia MurR/RpiR con dominios HTH y SIS en Tenericutes
- Familia LacI/PurR en Verrucomicrobia
]
.pull-right[
...
factores sigma.
5. Evaluar la contribución del contexto genómico de los factores transcripcionales y sigma
